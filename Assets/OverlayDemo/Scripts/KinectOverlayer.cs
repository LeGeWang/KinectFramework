...
//	public Vector3 BottomLeft;
	...
	void Update () 
	{
		...
		{
			this.GetComponent<Renderer>().material.mainTexture = manager.GetUsersClrTex();
...
//			Vector3 vUp = TopLeft - BottomLeft;
			...
			if(manager.IsUserDetected())
			{
				...
				if(manager.IsJointTracked(userId, iJointIndex))
				{
					...
					Vector3 localPos = new Vector3(scaleX * 10f - 5f, 0f, scaleY * 10f - 5f); // 5f is 1/2 of 10 - size of the plane
					Vector3 vPosOverlay = transform.TransformPoint(localPos);
					//Vector3 vPosOverlay = BottomLeft + ((vRight * scaleX) + (vUp * scaleY));
					
					if(debugText)
					{
						debugText.GetComponent<GUIText>().text = new Vector2(scaleX, scaleY).ToString();
					}
